...
/*   Updated: 2020/01/26 13:37:08 by ohachim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execute_load_index(t_process **process, t_global *global_data)
{
	...
	if (reg < 1 || reg > REG_NUMBER)
		return ;
	adress_value = ft_get_ind_value(global_data, ((*process)->process_cursor + ((first_arg + second_arg) % IDX_MOD) % MEM_SIZE)); // Check if this is how we idx_mode.
