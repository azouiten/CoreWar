...
/*   Updated: 2020/02/01 22:48:28 by ohachim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execute_store(t_process **process, t_global *global_data)
{
	...
	if (DEBUG)
		ft_printf("first_arg_value reg %d which contains value %#x\n", first_arg_value, (*process)->registries[first_arg_value - 1]);
	...
	if ((*process)->arg[1] == REG_CODE)
	{
		...
		if (DEBUG)
			ft_printf("second arg which is a reg is %d\n", second_arg_value);
	...
	if ((*process)->arg[1] == IND_CODE)
	{
		...
		if (DEBUG)
			ft_printf("second argument ind is %#x in decimal %d\n", second_arg_value, second_arg_value);
