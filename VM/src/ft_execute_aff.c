...
/*   Updated: 2020/02/19 00:03:25 by ohachim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_execute_aff(t_process **process, t_global *global_data)
{
	...
	if (!global_data->affs)
	{
		if (!(global_data->affs = ft_add_aff_node(((*process)->registries[reg - 1]) % 256, global_data)))
			ft_manage_error(global_data, MALLOC_FAIL, -1, 1);
	}
	else
	{
		while (global_data->affs->next)
			global_data->affs = global_data->affs->next;
		if (!(global_data->affs->next = ft_add_aff_node(((*process)->registries[reg - 1]) % 256, global_data)))
			ft_manage_error(global_data, MALLOC_FAIL, -1, 1);
	}
}
